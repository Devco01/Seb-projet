...
export async function DELETE(
  ...
  try {
    ...
    const facturesCount = await prisma.facture.count({ where: { clientId: id } });
    const paiementsCount = await prisma.paiement.count({ where: { clientId: id } });
    
    if (devisCount > 0 || facturesCount > 0 || paiementsCount > 0) {
      return NextResponse.json(
